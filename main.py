...
g = geocoder.ip("me")  #address of IP  
lat=g.latlng    
...
home = Tk()
home.title("Crash Detection System")
...
model =  load_model('C:/Users/anjan/OneDrive/Desktop/Accident Detection with UI/model/model.h5')

def Exit():
    global home
    result = messagebox.askquestion("Crash Detection System", 'Are you sure you want to exit?', icon="warning")
    if result == 'yes':
        home.destroy()
        exit()
    else:
    ...
    while 1:
            ...
            if 'No' not in txt:
                    if count>=50:
                            ...
                            txt = "Crash Detected at "+str(datetime.datetime.now())
                            message = """\
                            Subject: Alert !!! Crash
                            """+txt + " location : "+g[0] +" Latitude : " + lat
                            cl.messages.create(body=message, from_='+18317062001', to='+919441777477')
            ...
            cv2.imshow('Car Crash Detection',imgc)
...
def about():
    about = Toplevel()
    about.title("Crash Detection System")
